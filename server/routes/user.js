...
router.post('/add-user', async function (req, res, next) {
  try {
    ...
    let { firstname, lastname, email, department, user_password, confirm_password, user_level } = req.body;
    ...
    con.query(
      ...
      function (err, result, fields) {
        ...
        if (result.length > 0) {
          res.send({ status: 0, message: "Email address of user already exists!" });
          ...
          con.query(
            ...
            [firstname, lastname, email, department, hashed_password, user_level], function (err, result) {
              if (err) {
                console.log(err);
...
router.put('/:id', async function (req, res, next) {
  try {
    var sql;
    let { id, firstname, lastname, email, position, user_password, confirm_password, user_level } = req.body;
